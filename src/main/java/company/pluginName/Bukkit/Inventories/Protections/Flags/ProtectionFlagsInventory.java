		...
		if (flags.size() != 0) {
			...
			for (Flag<?> flag : Arrays.copyOfRange(flags.toArray(new Flag<?>[flags.size()]),
				...
				setSlot(slot++, new Button(ItemStacksUtils.createItemStack(
						ItemStacksUtils.setSkin(Material.PLAYER_HEAD.getItemStack(), flag.getItemData().getSkin()),
						MessageBuilder.createMessage(
								TextInput.inst().text(MessageString.INVENTORY_PROTECTION_FLAGS_FLAGNAME.toString())
										.replacements(new TextReplacement("{flag}", () -> flag.getItemData().getName()),
												new TextReplacement("{value}", () -> {
													if (flag.getType() == State.class) {
														return ((State) flag
																.getFlagValue(protectedRegion) == State.ALLOW
																		? MessageString.INVENTORY_PROTECTION_FLAGS_ALLOWVALUENAME
																				.toString()
																		: MessageString.INVENTORY_PROTECTION_FLAGS_DENYVALUENAME
																				.toString())
																.toString();
													} else if (flag.getType() == String.class) {
														String text = (String) flag.getFlagValue(protectedRegion);
														return MessageBuilder.createMessage(TextInput.inst().text(
																MessageString.INVENTORY_PROTECTION_FLAGS_STRINGVALUENAME
																		.toString())
																.replacements(new TextReplacement("{text}",
																		() -> text != null ? text : "---")))
																.toString();
													}
													return "&7???";
