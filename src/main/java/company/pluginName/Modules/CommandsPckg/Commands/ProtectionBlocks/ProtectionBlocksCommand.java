...
public class ProtectionBlocksCommand extends PandaCommand {
	...
	private static PandaBooleanField SETTINGS_OPENSHOPONLISTEMPTY = new PandaBooleanField(
			"Settings.Open-shop-on-empty-list", false);
	...
	public CommandResponse executeCommandProcess(PandaCommand cmd, CommandSender sender, String[] args,
		...
		if (pl != null && args.length == 0) {
			...
			if (protection != null && ProtectionUtilities.canManage(protection, pl)) {
				...
				if (vaultApi.isHooked() && SETTINGS_OPENSHOPONLISTEMPTY.getContent()
