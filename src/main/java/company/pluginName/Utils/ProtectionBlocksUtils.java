...
public class ProtectionBlocksUtils {
	...
	public static boolean isSameType(ItemStack protectionBlock, Block block) {
		...
		case VANILLA:
			...
			if (block.getType() == ItemUtilities.getPlayerHeadMaterial()
					&& protectionBlock.getType() == ItemUtilities.getPlayerHeadMaterial()) {
				return true;
			}
			break;
		}
	...
	public static Map<String, Object> protectionBlockToMap(ProtectionBlock protectionBlock) {
		...
		if (protectionBlock.getPermission() != null) {
			values.put(PERMISSION_SECTION, protectionBlock.getPermission());
		}

		if (protectionBlock.getPrice() != null) {
			values.put(PRICE_SECTION, protectionBlock.getPrice());
		}
