...
public class PlaceHolder extends PlaceholderExpansion {
	...
	private static final String FLAG_PREFIX = "protection_flag_";
	...
	public String onPlaceholderRequest(Player player, String identifier) {
		if (player != null) {
			if (identifier.toLowerCase().startsWith(FLAG_PREFIX)) {
				ProtectedRegion region = getRegionIn(player);
				if (region != null) {
					String flagName = identifier.substring(FLAG_PREFIX.length());
					try {
						Optional<Flag<?>> foundFlag = MainPluginClass.getWorldGuardAPI().getInternalWorldGuard()
								.getAllFlags().stream().filter(flag -> flag.getName().equals(flagName)).findFirst();
				...
				case "protection_size":
					...
					break;
				case "player_current":
					return String.valueOf(MainPluginClass.getPlugin().getProtectionsModule().getProtectionsByOwner()
	...
	private ProtectedRegion getRegionIn(Player player) {
		try {
			return MainPluginClass.getWorldGuardAPI().getInternalWorldGuard().getApplicableRegions(player.getLocation())
					.getRegions().stream().filter(region -> MainPluginClass.getPlugin().getProtectionsModule()
							.getProtectionByRegion().get(region.getId()) != null)
	...
	private Protection getProtectionIn(Player player) {
		try {
			Optional<ProtectedRegion> foundRegion = MainPluginClass.getWorldGuardAPI().getInternalWorldGuard()
					.getApplicableRegions(player.getLocation()).getRegions().stream().filter(region -> MainPluginClass
							.getPlugin().getProtectionsModule().getProtectionByRegion().get(region.getId()) != null)
					.findFirst();

			if (foundRegion.isPresent()) {
				return MainPluginClass.getPlugin().getProtectionsModule().getProtectionByRegion()
						.get(foundRegion.get().getId());
			}
		} catch (Exception e) {
		}
		return null;
