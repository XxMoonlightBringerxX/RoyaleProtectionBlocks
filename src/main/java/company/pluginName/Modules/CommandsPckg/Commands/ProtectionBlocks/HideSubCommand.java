...
@PandaCommandAnnotation(
		...
		pathName = "Hide",
		defaultName = "hide",
		defaultDescription = "Hide the protection block on your current protection",
		defaultAliases = "h")
	...
	public CommandResponse executeCommandProcess(CommandSender sender, PandaParameters parameters) {
		...
		if (pl != null) {
			Protection protection = RoyaleProtectionBlocksAPIImpl.getInstance().getProtectionsService()
					.findProtectionByLocation(pl.getLocation());
			if (protection != null) {
				try {
					playerInteractionsService.protectionHideBlockRequest(
							ProtectionHideBlockRequestInput.inst(pl, protection.getParentProtection()));
